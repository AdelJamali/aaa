...
* Strictly read-only (SELECT/SHOW/get_option) — zero writes, no patch, no behavior change.
* Tests: 10.12 workflow 17/17 + 10.11 regression + P0 governor — all PASS.

== 10.12.4 ==
* Version: 10.12.4 — Installable ZIP: `golden-importer-10.12.4.zip` (repo root).
* 🔬 SELECTION WINDOW AUDIT (read-only) added inside the 🩺 chain-audit tool (same one-click run, new section): scans the first 500 candidates of the EXACT production selection query (ORDER BY pi.id ASC — create_sessions L404-414), reads message_pk DIRECTLY from profile_items (not via JOIN) to prove its real value, re-runs the same 3-way JOIN + existing-session check of create_from_profile_item, classifies every candidate (NO_ITEM: NO_PK message_pk=0 / DANGLING_PK message-deleted / PROFILE_MISSING), reports window stats for first 20 / first 100 / first 500, counts orphans across ALL eligible (case A orphan-starvation vs B data-integrity vs C create-path), shows BROKEN vs HEALTHY candidate full-chain comparison, reconciles with the DB at that moment (last_run / max session / available id-range), and prints the final SELECTION AUDIT block (eligible_total / candidate_window / NO_ITEM / VALID / EXISTING / first_valid_candidate / FIRST HARD BREAK / Evidence / File / Function / Line) in a copyable panel.
* Strictly read-only: SELECT/SHOW/get_option only — zero INSERT/UPDATE/DELETE. No change to any business logic.
* Tests: 10.12 workflow 17/17 + 10.11 regression + P0 governor — all PASS.
